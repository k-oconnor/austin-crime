...
## Third Model: Random Forest (error out)
```{r Random Forest, message=FALSE, warning=FALSE, echo=FALSE}
trctrl <- trainControl(method = "cv", number = 5, savePredictions=TRUE)

crime_train$clearance_status <- factor(crime_train$clearance_status, labels = c("yes", "no"))
...
## Fifth Model: KNN Model (error out)
...
crime_knn1 <- train(clearance_status ~ population_density + median_income + median_home_value + prop_white + arrest_rate + zipcode,
                    data = crime_train,
                    method = "knn", trControl=trctrl, tuneLength=20)
...
## Sixth Model: Lasso Model (not done yet)
